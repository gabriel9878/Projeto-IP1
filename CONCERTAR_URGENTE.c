...
{
    int codProduto;
    int quantProduto;//SOMA DA QUANTIDAE DE TODOS OS PRODUTOS DE MESMO CÓDIGO;
...
//---------------------------------------[<FUNÇÕES & PROCEDIMENTOS>]-----------------------------------------//
    ...
    int positionProduto (Produto prodt[], int *pcq, int *pc);
    ...
    void buyProduto (Administrador admt[],Funcionario func[], Produto prodt[], Socio sc[], char login[], int *acq, int *fcq, int *pcq, int *scq, int *amax, int *fmax, int *pmax, int *smax);
...
//---------------------------------------[<PROCEDIMENTOS: ADMINISTRADOR>]-------------------------------------//
    ...
    void menuFuncionario (Administrador admt[],Funcionario func[], Produto prodt[], Socio sc[], char login[], int *acq, int *fcq, int *pcq, int *scq, int *amax, int *fmax, int *pmax, int *smax)
    {
        ...
        case 5:
            buyProduto (admt, func, prodt, sc, login, acq, fcq, pcq, scq, amax, fmax, pmax, smax);
            break;
    ...
    /*void buyProduto (Administrador admt[],Funcionario func[], Produto prodt[], Socio sc[], char login[], int *acq, int *fcq, int *pcq, int *scq, int *amax, int *fmax, int *pmax, int *smax)
    {
        ...
        //SOLICITAÇÃO E LEITURA DO CODIGO DO PRODUTO.
        printf("\n\nDigite um codigo para o produto: \n\n");
        fflush(stdin);
        scanf("%d", &prodtemp.cod);
        ...
        //SOLICITAÇÃO E LEITURA DA QUANTIDADE DE PRODUTO QUE SE DESEJA ADQUIRIR.
        printf("\n\nDigite a quantidade deste produto que deseja adquirir:\n\n");
        scanf("%d", &prodtemp.quantidade);

        //CHECAGEM DA POSSIBILIDADE DE VENDA CON BASE NA QUANTTIDADE DE PRODUTO ARMAZENADO.
        while (prodtemp.quantidade < prodt[positionProduto (prodt, pcq, prodtemp.cod)])
            for(i = 0; i < *pcq; i++)
            {
                if((positionProduto (prodt, pcq, prodtemp.cod)) == i)
                {
                    if(prodtemp.quantidade > (prodt[i]).quantidade)
                    {
                        printf("\n\nQuantidade deste produto armazenado nao suficiente para conclusao de transacao comercial!\n");
                        printf("\n\nForneca outro valor para a quantidade de produto requerido, ou procure um administrador!\n");
                        scanf("%d", &prodtemp.quantidade);
                    }
                }
            }
        comanda = buyProdutoCod (prodt, sc, prodtemp.cod, pcq, prodtemp.quantidade, Socio *sci(senha));
    }


    Socio * scSenha (char senha[10], int *scq)
    {
        Socio sctemp;
        int i = 0;
    ...
    //RETORNA A POSIÇÃO DO PRODUTO CADASTRADO.
    int positionProduto (Produto prodt[], int *pcq, int *pc);
    {
        int i = 0;
    ...
    int buyProdutoCod (Produto prodt[], Socio sc[], int *pc, int *pcq, int *quant, Socio *sci)
    {
        int i = 0;
        ...
        for(i = 0; i < *pcq; i++)
        {
            if(prodt[i].cod == *pc)
            {
                prodt[i].quantidade -= (*quant);

                sci->comp[sci->quantCompras].codProdt = *pc;

                sci->comp[sci->quantCompras].quantProdt = (*quant);

                sci->comp[sci->quantCompras].valorTotal = (*quant) * prodt[i].precoVenda;
    ...
    void init(Administrador admt[], Funcionario func[], Produto prodt[], Socio sc[], int *admtCadQtd,  int *funcCadQtd, int *prodtCadQtd, int *socioCadQtd)
    {
        ...
        *funcCadQtd = 2;
        ...
        sc[0] = (Socio)
        {"Alexandre","alx","6789",45698578423,{1,10,10},1
        };
        sc[1] = (Socio)
        {"Karina","kar","9876",15122018435,{1,10,10},1
        };
...
int main()
    {
        ...
        int COMPRA_MAX = 200;  //MAXIMO DE COMPRAS DO SOCIO
        ...
        /*positionProduto (Produto prodt[], int *pcq, int *cod);*/
